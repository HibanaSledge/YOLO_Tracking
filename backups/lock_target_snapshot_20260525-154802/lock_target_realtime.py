...
def session_saving_enabled(args: argparse.Namespace) -> bool:
    return not args.no_save_session
...
def processing_worker(args: argparse.Namespace, shared: SharedState) -> None:
    ...
    while not shared.stop_event.is_set():
        ...
        candidates = collect_candidates_realtime(result, frame, embed_model, face_detectors, classes, args, state, processed_index)
    ...
    if session_saving_enabled(args) and shared.session_paths is not None:
        ...
        summary = {
            ...
            "output_video": str(shared.session_paths.output_video),
            "frame_metrics_json": str(shared.session_paths.metrics_json),
            "selected_at_frame": selected_at_frame,
