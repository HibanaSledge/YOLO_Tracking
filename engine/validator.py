...
from ultralytics.utils.debug_vis import debug_enabled, ensure_debug_dir, set_debug_context, should_save_debug
...
class BaseValidator:
    ...
    def __init__(self, dataloader=None, save_dir=None, args=None, _callbacks: dict | None = None):
        ...
        self.debug_vis = debug_enabled(self.args)
    ...
    def __call__(self, trainer=None, model=None):
        ...
        for batch_i, batch in enumerate(bar):
            ...
            with dt[1]:
                visualize = False
                if self.debug_vis and RANK in {-1, 0} and should_save_debug(batch_i):
                    set_debug_context(model, self.save_dir, "val", batch_i, getattr(trainer, "epoch", None) + 1 if trainer else None)
                    visualize = ensure_debug_dir(self.save_dir, "val", "02_backbone_neck", f"batch_{batch_i:04d}")
            ...
            if self.debug_vis and RANK in {-1, 0} and should_save_debug(batch_i):
                counts = [int(len(x["conf"])) for x in preds]
                LOGGER.info(f"[debug] val batch {batch_i}: detections_per_image={counts}")
