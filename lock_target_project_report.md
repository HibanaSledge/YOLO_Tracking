...
- 已完成 G1-G7 常规离线调参、C0-C8 corner case 调参，以及 P0-P11 priority sweep 三阶段实验归档与中文分析。
- 已完成 `FACE_LOCK`、`HEAD_PROXY`、`LOST`、`SEARCHING` 等主要指标的文档化解释，并明确人工关键帧复核方法。
...
- 从多轮调参证据看，不能把 tracker id 连续性直接等同于业务身份连续性，也不能把 `HEAD_PROXY` 视为真实 `FACE_LOCK` 成功。
- 当前最值得继续复核和推进的候选方向集中在 P4、P5、P6、P11，但是否真正更优仍必须结合人工关键帧复核。
...
### 6.4 里程碑四：多阶段调参与报告体系成型

当前调参工作已经不再停留在单轮试错，而是形成了完整的三阶段证据链：

- G1-G7：先判断主检测分辨率、人脸召回、重绑定阈值、ReID 刷新和 MTCNN 刷新是不是主要瓶颈。
- C0-C8：在更具挑战的视频上验证 corner case，包括控制平滑、阈值和人脸相关参数。
- P0-P11：围绕优先级最高的 ReID 稀疏化、人脸尺度、人脸置信度、检测器 A/B、ReID A/B 做集中 sweep。

这意味着当前项目已经具备“提出假设 -> 运行实验 -> 统一落盘 -> 报告分析 -> 人工关键帧复核”的稳定调参工作流。
...
结合 G1-G7、C0-C8 和 P0-P11，可以得到当前更稳定的工程判断：

- `imgsz` 上探和更重检测器并没有证明能以合理代价换来足够的质量收益，至少在当前 CPU-only 环境下不应作为默认方向。
- 人脸尺度微调是当前最可能带来真实收益的质量方向，其中 P5 和 P6 值得优先人工复核。
- ReID 刷新稀疏化可以显著降低耗时，其中 P4 在速度上最有吸引力，但必须确认没有隐藏的身份漂移。
- ReID 模型从 `yolo26l.pt` 切到 `yolo26n.pt` 的 P11 显示出较强轻量化潜力，但同样不能只凭速度和表格数字下结论。
- 现阶段最重要的不是继续盲目扩参，而是先完成关键帧复核，确认哪些收益是真收益，哪些只是 `HEAD_PROXY` 或误锁带来的假收益。
...
如果按文档与证据成熟度来判断，项目当前已经具备稳定的实验和复盘能力，但还处在“候选默认配置待确认”的阶段。
...
- 先对 P5、P6 做人工关键帧复核，确认新增 `FACE_LOCK` 是否真的落在目标脸上。
...
- 优先复核 P4、P11，判断它们能否作为“轻量化但不过度伤质量”的候选默认方案。
...
1. 先完成 P4、P5、P6、P11 的人工关键帧复核，确认真实优先候选。
2. 把 lightweight 拆分成“只减 embedding”和“减 embedding + 减 MTCNN”两个版本重新对比，避免把不同来源的收益混在一起。
3. 补一套自动化几何质量评测指标，替代仅靠视频肉眼判断。
4. 在确认质量可接受后，再决定是否继续做更激进的轻量化，还是转向硬件升级。

建议把 [docs/tuning/offline_tuning_analysis_report.md](docs/tuning/offline_tuning_analysis_report.md) 作为当前调参主报告，把本报告视为项目级综述；两者职责应保持分层，而不是互相重复。
