...
## 记录 16：离线与实时统一加入性能指标记录

### 新增

- 新增 perf_utils.py，集中封装 PerformanceRecorder、逐帧性能记录结构和聚合统计逻辑。
- 新增离线与实时两种模式的 performance.json 输出，运行一次后会和原来的 summary.json、frame_metrics.json 一起落到同一 run_dir。
- 新增阶段级耗时指标，包括 detect_track_ms、collect_candidates_ms、face_detect_total_ms、face_detect_classical_ms、face_detect_profile_ms、face_detect_mtcnn_ms、embedding_ms、draw_overlay_ms、write_video_ms、show_ms。
- 新增逐帧资源采样指标，在环境支持时记录 system_cpu_percent、process_cpu_percent、process_rss_mb，以及 CUDA 显存占用指标。

### 减少或移除

- 不再需要通过人工拆解 summary 和 frame_metrics 去反推主检测、人脸检测和 ReID 的耗时来源。

### 技术细节

- 性能记录模块采用可选依赖方式实现：有 psutil 和 torch 时记录更完整的 CPU、内存和 CUDA 数据，没有时仍会保留阶段耗时和逐帧时延，不阻断主流程。
- 离线脚本在每一帧中记录主检测、候选生成、状态更新、控制更新、绘制、写视频和显示耗时，并在 summary 中新增 performance_json 路径。
- 实时脚本沿用同一套记录逻辑，并在 frame_records 中额外保留 source_frame_id 和 dropped_frames_before，便于将算法耗时与实时丢帧直接关联。

### 目的

- 让当前算法在离线和实时两种运行模式下都能在一次运行后自动留下性能证据，为后续判断硬件瓶颈、算法瓶颈和优化收益提供统一口径。

### 有效性证据与指标

- 证据来源：代码结构变更。
- 输出内容：每次运行现在都会额外生成 performance.json，并在 summary.json 中明确给出该文件路径。
- 指标覆盖：主检测耗时、人脸检测耗时、MTCNN 耗时、ReID 耗时、逐帧总耗时、有效 FPS、CPU 占用、进程 RSS、CUDA 显存占用。

### 修改后仍存在的问题

- 当前资源指标是否完整取决于运行环境是否安装 psutil，以及是否在 CUDA 环境下运行 torch。
- 这些性能数据属于软件运行侧指标，并不直接等价于整机板级功耗；如果后续需要真实功耗数据，仍需外接电流和功率测量链路。

### 证据缺口

- 本次改动完成后尚未在用户的真实离线和实时场景上各复跑一次，因此新的 performance.json 还没有实测样本留档。

## 记录 17：输出模块参数化与 demo-only 轻量模式

### 新增

- 新增 demo-only 参数，用于快速生成仅含最终 demo 视频的轻量输出。
- 新增 no-save-video、no-save-summary、no-save-frame-metrics、no-save-performance 四类独立开关，离线和实时脚本都可按需控制输出。
- 新增在关闭 performance 输出时自动切换到 NullPerformanceRecorder，避免无意义的性能报告写盘。

### 减少或移除

- 不再强制每次运行都写出 summary.json、frame_metrics.json、performance.json 三类指标文件。

### 技术细节

- 离线和实时脚本现在都支持两层控制：demo-only 作为快捷轻量模式，细粒度 no-save-* 参数作为显式模块化开关。
- summary 内的 output_video、frame_metrics_json、performance_json 字段会按当前开关真实反映是否生成对应文件，而不是固定写死路径。
- 在关闭 performance 输出时，算法主循环会切换到空性能记录器，仍保持同一调用接口，但不再写出性能报告文件。

### 目的

- 让用户可以在“完整分析模式”和“只出最终 demo 的轻量模式”之间快速切换，避免每次演示都带着全部指标写盘开销。

### 有效性证据与指标

- 证据来源：代码结构变更。
- 新增能力：离线和实时两条命令链路都可以通过 demo-only 直接只生成最终视频，或通过 no-save-* 对各类输出进行组合控制。

### 修改后仍存在的问题

- demo-only 轻量模式主要减少的是写盘和性能记录负担，并不会改变当前检测、人脸、ReID 这些核心算法开销。
- 如果后续需要进一步减轻实时负载，仍需要继续裁剪算法链路本身，而不仅是关闭指标输出。

### 证据缺口

- 本次参数化改动完成后尚未基于同一输入做完整模式和 demo-only 模式的耗时对比，因此轻量模式相对完整模式的实际提速幅度还需要下一轮实测补录。

## 记录 18：离线链路加入轻量化运行预设

### 新增

- 离线脚本新增 lightweight 参数，用于一键启用更轻的运行预设。
- 离线脚本新增 reid_interval、mtcnn_interval 参数，和实时脚本保持一致。
- 实时脚本新增 lightweight 快捷开关，统一离线与实时的轻量化入口。

### 减少或移除

- 不再要求当前已锁定目标在每一帧都强制刷新 embedding。
- 不再要求当前已锁定目标在每一帧都强制跑一次 MTCNN。

### 技术细节

- 在 lock_target.py 中，当前目标如果仍是同一 tracker id，则只按 reid_interval 刷新 embedding，其余帧直接复用几何连续性判定。
- 在 lock_target.py 中，当前目标如果仍是同一 tracker id，则只按 mtcnn_interval 允许 MTCNN 参与，其余帧只保留 OpenCV frontal/profile 检测路径。
- lightweight 预设当前会将 reid_interval 至少提升到 8，将 mtcnn_interval 至少提升到 3。
- is_same_tracker_candidate_valid 现在支持“当前候选没有新 embedding”时的弱几何兜底，避免仅因为跳过一次 embedding 刷新就把同一目标误判为失配。

### 目的

- 直接削减离线主瓶颈 collect_candidates 中最重的两段 CPU 开销：embedding 提取和 MTCNN 调用。
- 在不改动总体锁定状态机的前提下，把离线链路的行为尽量靠近实时链路已经验证过的轻量化思路。

### 有效性证据与指标

- 证据来源：当前 performance.json 已表明 collect_candidates_ms、face_detect_total_ms、embedding_ms 是离线主瓶颈。
- 本次改动直接命中这三类统计中的前两项核心触发路径，理论上会优先降低同目标稳定跟踪阶段的单帧成本。

### 修改后仍存在的问题

- 这次改动主要优化的是“稳定跟踪同一目标”阶段；如果进入频繁重绑定场景，embedding 和人脸检测开销仍然会明显上升。
- 当前还没有基于同一输入样本补跑新的 performance.json，因此提速幅度仍需实测确认。

### 证据缺口

- 尚未完成修改前后同一视频、同一参数集下的 performance.json 对比。
- 尚未完成修改前后锁定几何质量的一致性复核。
