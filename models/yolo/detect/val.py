...
class DetectionValidator(BaseValidator):
    ...
    def preprocess(self, batch: dict[str, Any]) -> dict[str, Any]:
        ...
        if debug_enabled(self.args) and RANK in {-1, 0} and should_save_debug(self.batch_i):
            prefix = debug_file_prefix("val", self.batch_i)
            debug_dir = ensure_debug_dir(self.save_dir, "val", "01_preprocess")
            log_tensor_stats(f"{prefix}/img", batch["img"])
            save_tensor_npy(batch["img"], debug_dir / f"{prefix}_img.npy")
            save_tensor_image(
                batch["img"],
                debug_dir / f"{prefix}_first.jpg",
                title="VAL PREPROCESS: FIRST IMAGE",
                lines=[
                    f"batch={self.batch_i}",
                    f"shape={tuple(batch['img'].shape)}",
                    f"min={float(batch['img'].min()):.4f} max={float(batch['img'].max()):.4f}",
                ],
            )
            plot_images(
                labels={},
                ...
                fname=debug_dir / f"{prefix}_grid.jpg",
                names=self.names,
                save=True,
            )
            annotate_image_file(
                debug_dir / f"{prefix}_grid.jpg",
                "VAL PREPROCESS: GRID",
                [
                    f"batch={self.batch_i}",
                    f"batch_size={int(batch['img'].shape[0])}",
                    "Grid shows all images in this validation batch; FIRST shows only image[0].",
                ],
            )
            self._debug_batch_images = batch["img"].detach().cpu()
        return batch
    ...
    def postprocess(self, preds: torch.Tensor) -> list[dict[str, torch.Tensor]]:
        ...
        debug_now = debug_enabled(self.args) and RANK in {-1, 0} and should_save_debug(self.batch_i)
        if debug_now:
            prefix = debug_file_prefix("val", self.batch_i)
            debug_dir = ensure_debug_dir(self.save_dir, "val", "05_nms")
            log_tensor_stats(f"{prefix}/raw_preds", raw_preds)
            save_tensor_npy(raw_preds, debug_dir / f"{prefix}_before_nms.npy")
            set_nms_debug(True, prefix)
        ...
        if debug_now:
            counts = [int(len(x)) for x in outputs]
            LOGGER.info(f"[debug] {prefix}: NMS kept detections per image={counts}")
            save_tensor_npy(
                np.array([x.detach().float().cpu().numpy() for x in outputs], dtype=object),
                debug_dir / f"{prefix}_after_nms.npy",
            )
            image0 = getattr(self, "_debug_batch_images", None)
            if image0 is not None and len(image0):
                pred0 = raw_preds[0].detach()
                score0 = pred0[4:].amax(0)
                topk = min(50, pred0.shape[1])
                idx = score0.topk(topk).indices
                save_boxes_overlay(
                    image0[0],
                    pred0[:4, idx].transpose(0, 1),
                    debug_dir / f"{prefix}_decode_topk.jpg",
                    title="VAL DECODE: TOP-K BOXES BEFORE NMS",
                    scores=score0[idx],
                    box_format="xywh",
                    max_boxes=topk,
                )
                if len(outputs) and len(outputs[0]):
                    save_boxes_overlay(
                        image0[0],
                        outputs[0][:, :4],
                        debug_dir / f"{prefix}_nms_kept.jpg",
                        title="VAL NMS: BOXES AFTER NMS",
                        scores=outputs[0][:, 4],
                        box_format="xyxy",
                        max_boxes=min(50, len(outputs[0])),
                    )
            set_nms_debug(False)
    ...
    def plot_val_samples(self, batch: dict[str, Any], ni: int) -> None:
        ...
        if debug_enabled(self.args) and RANK in {-1, 0} and should_save_debug(ni):
            cls_count = int(batch["cls"].shape[0]) if isinstance(batch.get("cls"), torch.Tensor) else 0
            LOGGER.info(f"[debug] val batch {ni}: images={len(batch['im_file'])} gt_labels={cls_count}")
            plot_images(
                labels=batch,
                paths=batch["im_file"],
                fname=ensure_debug_dir(self.save_dir, "val", "06_plots") / f"val_batch{ni}_labels.jpg",
                names=self.names,
                on_plot=self.on_plot,
            )
    ...
    def plot_predictions(
        ...
        if debug_enabled(self.args) and RANK in {-1, 0} and should_save_debug(ni):
            pred_count = int(sum(len(x["conf"]) for x in preds))
            LOGGER.info(f"[debug] val batch {ni}: predicted_boxes={pred_count}")
            plot_images(
                images=batch["img"],
                labels=batched_preds,
                paths=batch["im_file"],
                fname=ensure_debug_dir(self.save_dir, "val", "06_plots") / f"val_batch{ni}_pred.jpg",
                names=self.names,
                on_plot=self.on_plot,
            )
