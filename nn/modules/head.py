...
from ultralytics.utils.debug_vis import (
    debug_file_prefix,
    ensure_debug_dir,
    get_debug_context,
    log_tensor_stats,
...
class Detect(nn.Module):
    ...
    def forward_head(
        ...
        box_outputs = [box_head[i](x[i]) for i in range(self.nl)]
        cls_outputs = [cls_head[i](x[i]) for i in range(self.nl)]
        boxes = torch.cat([box_outputs[i].view(bs, 4 * self.reg_max, -1) for i in range(self.nl)], dim=-1)
        scores = torch.cat([cls_outputs[i].view(bs, self.nc, -1) for i in range(self.nl)], dim=-1)
        context = get_debug_context(self)
        if context and should_save_debug(context.get("batch_i")):
            prefix = debug_file_prefix(context["stage"], context["batch_i"], context.get("epoch"))
            debug_dir = ensure_debug_dir(context["save_dir"], context["stage"], "03_head_raw")
            for i, feat in enumerate(x):
                log_tensor_stats(f"{prefix}/P{i + 3}", feat)
                save_tensor_npy(feat[0], debug_dir / f"{prefix}_p{i + 3}_feat.npy")
                save_feature_heatmap(
                    feat,
                    debug_dir / f"{prefix}_p{i + 3}_heatmap.jpg",
                    title=f"{context['stage'].upper()} HEAD RAW: P{i + 3} FEATURE HEATMAP",
                    line=f"shape={tuple(feat.shape)}",
                )
                cls_map = cls_outputs[i][0].sigmoid().amax(0)
                save_feature_heatmap(
                    cls_map,
                    debug_dir / f"{prefix}_p{i + 3}_scoremap.jpg",
                    title=f"{context['stage'].upper()} HEAD RAW: P{i + 3} SCORE MAP",
                    line=f"shape={tuple(cls_outputs[i].shape)} max(sigmoid(cls))",
                )
            log_tensor_stats(f"{prefix}/raw_boxes", boxes)
            log_tensor_stats(f"{prefix}/raw_scores", scores)
            save_tensor_npy(boxes, debug_dir / f"{prefix}_raw_boxes.npy")
            save_tensor_npy(scores, debug_dir / f"{prefix}_raw_scores.npy")
            score_matrix = scores[0].sigmoid()
            if score_matrix.shape[0] > 32:
                top_cls = score_matrix.max(dim=1).values.topk(32).indices
                score_matrix = score_matrix[top_cls]
                cls_note = "top32 classes by max score"
            else:
                cls_note = "all classes"
            save_matrix_heatmap(
                score_matrix,
                debug_dir / f"{prefix}_raw_scores_matrix.jpg",
                title=f"{context['stage'].upper()} HEAD RAW: SCORE MATRIX",
                line=f"shape={tuple(scores.shape)} {cls_note}",
            )
    ...
    def _get_decode_boxes(self, x: dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        dfl_boxes = self.dfl(x["boxes"])
        dbox = self.decode_bboxes(dfl_boxes, self.anchors.unsqueeze(0)) * self.strides
        context = get_debug_context(self)
        if context and should_save_debug(context.get("batch_i")):
            prefix = debug_file_prefix(context["stage"], context["batch_i"], context.get("epoch"))
            debug_dir = ensure_debug_dir(context["save_dir"], context["stage"], "04_decode")
            log_tensor_stats(f"{prefix}/anchors", self.anchors)
            log_tensor_stats(f"{prefix}/strides", self.strides)
            log_tensor_stats(f"{prefix}/dfl_boxes", dfl_boxes)
            log_tensor_stats(f"{prefix}/decoded_boxes", dbox)
            save_tensor_npy(self.anchors, debug_dir / f"{prefix}_anchors.npy")
            save_tensor_npy(self.strides, debug_dir / f"{prefix}_strides.npy")
            save_tensor_npy(dfl_boxes, debug_dir / f"{prefix}_dfl_boxes.npy")
            save_tensor_npy(dbox, debug_dir / f"{prefix}_decoded_boxes.npy")
        return dbox
