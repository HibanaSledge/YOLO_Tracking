...
from ultralytics.utils.debug_vis import get_nms_debug
...
def non_max_suppression(
    ...
    for xi, (x, xk) in enumerate(zip(prediction, xinds)):  # image index, (preds, preds indices)
        debug_state = get_nms_debug()
        debug_prefix = f"{debug_state.get('prefix', 'nms')} img{xi}"
        raw_count = int(x.shape[0])
        # Apply constraints
        ...
        if debug_state["enabled"]:
            LOGGER.info(f"[debug][nms] {debug_prefix}: raw={raw_count} conf_candidates={int(filt.sum())}")
        ...
        if multi_label:
            ...
            if return_idxs:
                xk = xk[filt]
        if debug_state["enabled"]:
            LOGGER.info(f"[debug][nms] {debug_prefix}: after_label_filter={int(x.shape[0])}")
        ...
        if classes is not None:
            ...
            if return_idxs:
                xk = xk[filt]
        if debug_state["enabled"]:
            LOGGER.info(f"[debug][nms] {debug_prefix}: after_class_filter={int(x.shape[0])}")
        ...
        if n > max_nms:  # excess boxes
            ...
            if return_idxs:
                xk = xk[filt]
        if debug_state["enabled"]:
            LOGGER.info(f"[debug][nms] {debug_prefix}: before_nms={int(x.shape[0])}")
        ...
        output[xi] = x[i]
        if debug_state["enabled"]:
            LOGGER.info(f"[debug][nms] {debug_prefix}: kept={int(len(i))}")
        if return_idxs:
