...
run_dir = root / 'runs' / 'lock_target' / 'hybrid_fix_6'
...
interesting = [1]
for item in data:
    if item.get('lock_mode') == 'HEAD_PROXY':
...
for item in data:
    if item.get('deadband_active'):
        ...
        break
for item in data:
    if item.get('lock_mode') == 'LOST' or item['state'] == 'LOST':
        interesting.append(item['frame_index'])
        lost_idx = item['frame_index']
        break
else:
    lost_idx = None
if lost_idx is not None:
    for item in data:
        if item['frame_index'] > lost_idx and item['state'] == 'REACQUIRE':
            interesting.append(item['frame_index'])
            break
...
for img_path in saved:
    ...
    cards.append(f"""
    <section class='card'>
      <h2>Frame {frame_idx} | state={item['state']} | lock_mode={item.get('lock_mode')} | visible={item['visible']} | tracker_id={item['tracker_id']}</h2>
...
<head>
<meta charset='utf-8' />
<title>hybrid_fix_6 inspection</title>
<style>
